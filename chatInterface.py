...
def ChatBotWithHistory():
    ...
    layout = [[sg.Output(size=(127, 30), font=('Helvetica 10'))],
              [sg.Text('Saida'),
               sg.Text('', size=(20, 3), key='history')],
              [sg.ML(size=(85, 5), enter_submits=True, key='query', do_not_clear=False),
               sg.Button('Enviar', button_color=(sg.YELLOWS[0], sg.GREENS[0]), bind_return_key=True),
               sg.Button('Sair', button_color=(sg.YELLOWS[0], sg.GREENS[0]))]]

    window = sg.Window('Sistema de chat', layout,
